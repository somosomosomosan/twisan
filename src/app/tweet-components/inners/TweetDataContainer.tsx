...
import { fptsFindFirstSomeCallback } from '../../util-funcs/array';
...
import { COLOR_BG_IN_DARKMODE, COLOR_BORDER, DUMMY_AUTHOR, DUMMY_TWEET } from './consts';
import {
	t_cardLink,
	t_dbAuthor,
	t_dbTweetDataParsed,
...
export default function TweetDataContainer(props: {
...
function QuotedContainer(props: {
	...
	const tweetData = pipe(
		props.loadTweetData(props.tweetId),
		O.getOrElse(() => DUMMY_TWEET),
	);
	const authorData = pipe(
		props.loadAuthorData(tweetData.author_id),
		O.getOrElse(() => DUMMY_AUTHOR),
	);
...
function RepliedContainer(props: {
	...
	const _loadTweetData = (): t_dbTweetDataParsed =>
		pipe(
			[() => props.loadTweetData(props.tweetId), () => props.loadTweetData(props.conversationId ?? '')],
			fptsFindFirstSomeCallback,
			O.getOrElse(() => DUMMY_TWEET),
		);
